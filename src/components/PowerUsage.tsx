...
// Replace your existing stateOptions and countryOptions derivations with:
const stateOptions: string[] = electricityData
  ? Array.from(new Set(electricityData.map((item: ElectricityPrice) => item.state))).sort()
  : [];

const countryOptions: string[] = internationalData
  ? Array.from(new Set(internationalData.map((item: InternationalElectricityPrice) => item.country))).sort()
  : [];
