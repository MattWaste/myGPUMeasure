...
const stateOptions: string[] = Array.from(
  new Set(electricityData ? electricityData.map((item: ElectricityPrice) => String(item.state)) : [])
).sort();

const countryOptions: string[] = Array.from(
  new Set(internationalData ? internationalData.map((item: InternationalElectricityPrice) => String(item.country)) : [])
).sort();
